...
public class ConstructorThread implements Runnable {
    ...
    public ConstructorThread(SnakeBoard snakeBoard) {
        super();
    ...
    public void run() {
        while(!stop){
            ...
            createCoins();
            try {
                if (snakeBoard.getGameBest() < Environment.getInstance().LIFEINCREASING){
                    Thread.sleep(Environment.getInstance().MINTHREADDELAY);
                }
                else {
                    int actualShift = (snakeBoard.getGameBest())/Environment.getInstance().LIFEINCREASING;
    ...
    private void createBlocks(){
        ...
        for(int i=0; i<7; i++){
            ...
            int gameBest = snakeBoard.getGameBest();
